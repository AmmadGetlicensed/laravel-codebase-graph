...
def _validate_schema(code: str) -> ValidationResult:
    ...
    # Detect deprecated exists() predicate (KuzuDB requires EXISTS { MATCH ... } subquery)
    for node in _ast.walk(tree):
        if isinstance(node, _ast.Constant) and isinstance(node.value, str):
            s = node.value.strip()
            if _re.search(r'\bexists\s*\(', s, _re.IGNORECASE):
                errors.append(
                    "Deprecated exists() predicate detected. "
                    "Use EXISTS { MATCH ... } subquery syntax instead. "
                    f"Snippet: {s[:120]}"
                )
...
def _generate_plugin_spec(
    ...
    prompt = (
        ...
        "- NEVER use exists() predicate — it is deprecated in KuzuDB; use EXISTS { MATCH ... } subquery syntax\n"
        "  Example: WHERE EXISTS { MATCH (n)-[:CALLS]->(m:EloquentModel {name: 'User'}) }\n"
        + (
...
def _rewrite_exists_predicates(source: str) -> tuple[str, int]:
    """Rewrite deprecated ``exists(...)`` predicates to ``EXISTS { MATCH ... }`` subquery syntax.

    KuzuDB deprecated the old ``WHERE exists((n)--(:Label))`` form.
    The new syntax is ``WHERE EXISTS { MATCH (n)--(:Label) }``.

    Also fixes the common LLM mistake of placing node property filters *after* the
    closing paren — ``(:Label){prop: val}`` → ``(:Label {prop: val})``.

    Returns:
        (rewritten_source, number_of_replacements)
    """
    result: list[str] = []
    count = 0
    i = 0
    pat = _re.compile(r'\bexists\s*\(', _re.IGNORECASE)
    ...
    while i < len(source):
        ...
        inner = source[m.end():j - 1]  # content between exists( and )

        # Fix (:Label){prop: val} → (:Label {prop: val}) inside the inner pattern
        inner_fixed = _re.sub(r'\)\s*(\{[^{}]*\})', r' \1)', inner)

        result.append(f"EXISTS {{ MATCH {inner_fixed} }}")
        count += 1
        i = j
...
def migrate_plugin_cypher_properties(plugin_path: Path) -> list[str]:
    """Fix known wrong Cypher property names and deprecated syntax in an existing plugin file.
    ...
    Also rewrites deprecated ``exists(...)`` predicates to ``EXISTS { MATCH ... }``
    subquery syntax.
    ...
    # Rewrite deprecated exists() predicates
    rewritten, exists_count = _rewrite_exists_predicates(updated)
    if exists_count:
        updated = rewritten
        applied.append(f"exists() predicate → EXISTS {{MATCH}} subquery ({exists_count}x)")
