...
def agent_install(
    ...
    reads at session start — tool hierarchy, investigation workflows, and
    common pitfalls.
...
def tail(
    ...
    mgr.tail(callback=_print_entry, level=level, tool=tool)
...
def doctor(path: Optional[Path] = PathArg) -> None:
    ...
    Runs a comprehensive diagnostic and reports pass/warn/fail for each check:
     ...
     11. Logs                — LogManager read/filter/stats smoke test
     12. Downloads           — bundled model/asset download status
